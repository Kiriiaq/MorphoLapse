...
class InputValidator:
    ...
    def validate_directory(
        ...
        return ValidationResult(valid=True, level=ValidationLevel.INFO, message="Dossier valide", field="directory")

    @classmethod
    def validate_file(
        cls, path: str, must_exist: bool = True, allowed_extensions: set | None = None, max_size_mb: float | None = None
    ) -> ValidationResult:
        """
        ...
        if must_exist and not os.path.exists(path):
            return ValidationResult(
                valid=False, level=ValidationLevel.ERROR, message=f"Fichier non trouvé: {path}", field="file"
            )

        if must_exist and not os.path.isfile(path):
            return ValidationResult(
                valid=False, level=ValidationLevel.ERROR, message=f"N'est pas un fichier: {path}", field="file"
            )
        ...
        return ValidationResult(valid=True, level=ValidationLevel.INFO, message="Fichier valide", field="file")
    ...
    def validate_numeric(
        ...
        return ValidationResult(
            valid=True, level=ValidationLevel.INFO, message=f"{param_name} valide: {num_value}", field=param_name
        )

    @classmethod
    def validate_disk_space(cls, path: str, required_mb: float) -> ValidationResult:
        """
    ...
    def validate_ffmpeg(cls) -> ValidationResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return ValidationResult(
                    valid=True, level=ValidationLevel.INFO, message=f"FFmpeg trouvé (version {version})", field="ffmpeg"
                )
            ...
            return ValidationResult(
                valid=False, level=ValidationLevel.WARNING, message="FFmpeg ne répond pas", field="ffmpeg"
            )
        except Exception as e:
            return ValidationResult(
                valid=False, level=ValidationLevel.ERROR, message=f"Erreur FFmpeg: {e}", field="ffmpeg"
            )

        return ValidationResult(
            valid=False, level=ValidationLevel.ERROR, message="FFmpeg non fonctionnel", field="ffmpeg"
        )
    ...
    def validate_model_file(cls, path: str) -> ValidationResult:
        ...
        if not path:
            ...
            for sp in search_paths:
                if os.path.exists(sp):
                    return ValidationResult(
                        valid=True, level=ValidationLevel.INFO, message=f"Modèle trouvé: {sp}", field="model"
                    )
        ...
        if not os.path.exists(path):
            return ValidationResult(
                valid=False, level=ValidationLevel.ERROR, message=f"Fichier modèle non trouvé: {path}", field="model"
            )
        ...
        return ValidationResult(valid=True, level=ValidationLevel.INFO, message="Modèle valide", field="model")
...
class WorkflowValidator:
    ...
    def validate_all(self) -> tuple[bool, list[ValidationResult]]:
        ...
        if self.context.output_dir:
            result = InputValidator.validate_directory(self.context.output_dir, must_exist=False, must_be_writable=True)
            results.append(result)
        ...
        for param in ["fps", "transition_duration", "pause_duration", "border_size"]:
            if param in self.context.config:
                result = InputValidator.validate_numeric(self.context.config[param], param)
                results.append(result)
        ...
        if self.context.input_dir and os.path.exists(self.context.input_dir):
            num_images = len(
                [
                    f
                    for f in os.listdir(self.context.input_dir)
        ...
        self.errors = [
            r for r in results if not r.valid and r.level in (ValidationLevel.ERROR, ValidationLevel.CRITICAL)
        ]
...
def read_file_with_encoding_fallback(file_path: str, encodings: list[str] = None) -> tuple[str | None, str, str | None]:
    """
